...
├── publish_snapshot.py    # Throttled VPS/local publisher for GitHub artifacts
...
└── .github/workflows/
    └── scrape.yml         # CI/smoke checks; does not overwrite snapshots
```
...
# HTTP session-quality check: the same proxy must keep one observed egress IP + country
...
`.github/workflows/scrape.yml` is CI/smoke validation only. It checks syntax and verifies committed snapshot files are usable. It intentionally does **not** scrape or overwrite snapshots, because GitHub runner network conditions produce smaller/lower-quality pools.

High-quality snapshots should be generated by a VPS/local agent with the 1500-sample `freshen_pool.py` command, then committed to `main`.

## VPS publisher mode

Use `publish_snapshot.py` after a successful VPS/local refresh. It exports final
snapshots from `data/proxies.db`, checks quality gates, stages only generated
artifacts, and pushes to GitHub when changes are meaningful.
...
Use `--force` only when intentionally publishing a reset/baseline snapshot.
